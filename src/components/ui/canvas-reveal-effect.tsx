 ...
 const uniforms: Uniforms = useMemo(() => {
   ...
   return {
     u_colors: {
       ...
       type: "uniform3fv", // <-- use literal, not string
     },
     u_opacities: {
       value: opacities,
       type: "uniform1fv", // <-- literal
     },
     u_total_size: {
       value: totalSize,
       type: "uniform1f", // <-- literal
     },
     u_dot_size: {
       value: dotSize,
       type: "uniform1f", // <-- literal
...
export const ShaderMaterial: React.FC<ShaderMaterialProps> = ({
  ...
  // Prepare uniforms in a type-safe way
...
const preparedUniforms = useMemo(() => {
  ...
  Object.entries(uniforms).forEach(([key, uniform]) => {
    switch (uniform.type) {
      case "uniform1f":
        u[key] = { value: uniform.value as number };
        break;
      case "uniform1fv":
        u[key] = { value: uniform.value as number[] };
        break;
